function [thetawf, thetadf] = estimatewd3(X, t, V, Vt, iter, rate, lambdaws, lambdads)
...
for j = 1:length(lambdaws)
	...
	lambdaw = lambdaws(j);
	for i = 1:iter
		[Jw, gradientw] = costwd(X, t(:,2), thetaw, lambdaw );
		Jws(i) = Jw;
		thetaw = thetaw - rate * gradientw;
	end
	[JVw1, ignore] = costwd(V, Vt(:,2), thetaw, 0 );
	JVws(j) = JVw1;
	if JVw1 < JVwt
		JVwt = JVw1;
		thetawf = thetaw;
...
[JVd0, ignore] = costwd(Vd, Vt(:,1), thetad, 0 );
...
for j = 1:length(lambdads)
	...
	lambdad = lambdads(j);
	for i = 1:iter
		[Jd, gradientd] = costwd(Xd, t(:,1), thetad, lambdad );
		Jds(i) = Jd;
		thetad = thetad - rate * gradientd;
	end
	[JVd1, ignore] = costwd(Vd, Vt(:,1), thetad, 0 );
	JVds(j) = JVd1;
	if JVd1 < JVdt
		JVdt = JVd1;
		thetadf = thetad;
...
[Jw, ignore] = costwd(X, t(:,2), thetawf, 0 );
[Jd, ignore] = costwd(Xd, t(:,1), thetadf, 0 );
